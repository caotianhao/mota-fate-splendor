...
class actions {
    ...
    _sys_onkeyUp_replay(e) {
        if (this._checkReplaying()) {
            ...
            else if (e.keyCode == 90) core.speedDownReplay();
            else if (e.keyCode == 67) core.speedUpReplay();
            else if (e.keyCode == 32) core.triggerReplay();
            else if (e.keyCode == 65) core.rewindReplay();
            else if (e.keyCode == 83) core.control._replay_SL();
            else if (e.keyCode == 88) core.control._replay_book();
            else if (e.keyCode == 33 || e.keyCode == 34)
                core.control._replay_viewMap();
            else if (e.keyCode == 78) core.stepReplay();
            else if (e.keyCode == 84) core.control._replay_toolbox();
            else if (e.keyCode == 81) core.control._replay_equipbox();
            else if (e.keyCode == 66) core.ui._drawStatistics();
            else if (e.keyCode >= 49 && e.keyCode <= 51)
                core.setReplaySpeed(e.keyCode - 48);
            else if (e.keyCode == 52) core.setReplaySpeed(6);
            else if (e.keyCode == 53) core.setReplaySpeed(12);
            else if (e.keyCode == 54) core.setReplaySpeed(24);
            return true;
        }
    }
    _sys_onkeyUp(e) {
        ...
        if (isArrow && !core.status.lockControl) {
            for (var ii = 0; ii < core.status.holdingKeys.length; ii++) {
                if (core.status.holdingKeys[ii] === e.keyCode) {
                    ...
                    if (ii === core.status.holdingKeys.length &&
    ...
    _sys_onmove(x, y, px, py) {
        ...
        if (core.status.preview.prepareDragging) {
            if (Math.abs(px - core.status.preview.px) <= 20 &&
    ...
    _sys_longClick_lockControl(x, y, px, py) {
        ...
        if (core.status.event.id == "fly") {
            if ((x == core._WIDTH_ - 2 || x == core._WIDTH_ - 3) &&
                (y == this._HY_ - 1 || y == this._HY_ + 3)) {
                this._clickFly(x, y);
                return true;
            }
        }
        if (["save", "load", "replayLoad", "replayRemain", "replaySince"].indexOf(
    ...
    _clickConfirmBox(x, y, px, py) {
        if (px >= core._PX_ / 2 - 70 &&
            px <= core._PX_ / 2 - 10 &&
            py >= core._PY_ / 2 &&
            py <= core._PY_ / 2 + 64 &&
            core.status.event.data.yes)
            core.status.event.data.yes();
        if (px >= core._PX_ / 2 + 10 &&
            px <= core._PX_ / 2 + 70 &&
            py >= core._PY_ / 2 &&
            py <= core._PY_ / 2 + 64 &&
    ...
    _clickAction(x, y, px, py) {
        ...
        if (core.status.event.data.type == "choices") {
            ...
            if (y >= topIndex && y < topIndex + choices.length) {
                ...
                clearTimeout(core.status.event.interval);
                var timeout = Math.max(0, core.status.event.timeout - new Date().getTime()) ||
                    0;
                delete core.status.event.timeout;
                core.setFlag("timeout", timeout);
                // 对全局商店特殊处理
        ...
        if (core.status.event.data.type == "confirm") {
            if ((x == this._HX_ - 2 || x == this._HX_ - 1) && y == this._HY_ + 1) {
                ...
                core.doAction();
            } else if ((x == this._HX_ + 2 || x == this._HX_ + 1) &&
                y == this._HY_ + 1) {
                clearTimeout(core.status.event.interval);
                var timeout = Math.max(0, core.status.event.timeout - new Date().getTime()) ||
                    0;
    ...
    _clickViewMaps(x, y, px, py) {
        ...
        if (py <= 2 * perpy &&
            ...
            core.playSound("光标移动");
            index++;
            while (index < core.floorIds.length &&
                index != now &&
    ...
    _clickToolbox(x, y) {
        ...
        if (x == this._HX_ + 2 || x == this._HX_ + 3) {
            if (y === core._HEIGHT_ - 1 - 5 &&
                toolsPage < Math.ceil(tools.length / this.LAST)) {
                core.status.event.data.toolsPage++;
                core.playSound("光标移动");
                core.ui._drawToolbox(core.status.event.selection);
            }
            if (y === core._HEIGHT_ - 1 &&
    ...
    _keyDownToolbox(keycode) {
        ...
        var toolsLastIndex = toolsPage < toolsTotalPage
            ? last_index
            : (tools.length + last_index) % this.LAST;
        var constantsLastIndex = this.LAST +
        ...
        if (keycode == 39) {
            ...
            if (toolsPage < toolsTotalPage && index == last_index) {
                core.status.event.data.toolsPage++;
                index = 0;
            } else if (constantsPage < constantsTotalPage &&
                index == 2 * this.LAST - 1) {
                core.status.event.data.constantsPage++;
                index = this.LAST;
    ...
    _clickCursor(x, y, px, py) {
        if (x == core.status.automaticRoute.cursorX &&
